...
function computeAggregates(rows: Array<{ date: string; campaign_name: string; runrate: number }>) {
  const platformSpendMap: Record<string, { allocated: number; spent: number }> = {}
  for (const d of rows) {
    const platform = detectPlatform(d.campaign_name)
    if (!platformSpendMap[platform]) platformSpendMap[platform] = { allocated: 0, spent: 0 }
    platformSpendMap[platform].spent += d.runrate * 10
    platformSpendMap[platform].allocated += d.runrate * 20
  }
  const platformSpend = Object.entries(platformSpendMap).map(([platform, v]) => ({
    platform,
  ...
  const totalBudget = platformSpend.reduce((acc, curr) => acc + curr.allocated, 0)
...
export default function BudgetPage() {
  const [state, setState] = React.useState(() => ({ platformSpend: [] as any[], budgetMonthly: [] as any[], totalBudget: 0, totalSpent: 0 }))
  React.useEffect(() => {
    const load = async () => {
      ...
      const rows = (json.data || []) as Array<{ date: string; campaign_name: string; runrate: number }>
  ...
  return (
    <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="p-4">
          <div className="space-y-2">
            ...
            <p className="text-2xl font-bold">{totalBudget ? ((totalSpent / totalBudget) * 100).toFixed(1) : "0.0"}%</p>
          </div>
        </Card>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          <div className="p-4">
            <h3 className="text-xl font-semibold mb-4">Budget vs Spend Trend</h3>
            <Chart
              data={budgetMonthly}
              xAxis="month"
              series={[
                ...
                <div key={platform.platform} className="space-y-2">
                  ...
                  <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{ width: `${(platform.spent / platform.allocated) * 100}%` }}
                    />
                  </div>
                </div>
              ))}
